...
class StateManagerTests(unittest.TestCase):
    def setUp(self):
        # The legacy-name tests must not inherit APP_CODE from the runner env.
        patcher = patch.dict(os.environ, {}, clear=False)
        patcher.start()
        os.environ.pop("APP_CODE", None)
    ...
    def test_app_code_scopes_state_into_app_folder(self):
        with tempfile.TemporaryDirectory() as directory:
            state_dir = Path(directory)
            with patch("common.state_manager.STATE_DIR", state_dir), patch.dict(
                os.environ, {"APP_CODE": "airlines70"}
            ):
